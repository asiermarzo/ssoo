...
Al pulsar `Ctrl+C`, el *driver* de terminal solicita al kernel que envíe `SIGINT` al grupo de procesos en primer plano. Por eso basta con instalar un manejador de `SIGINT` para que un programa ignore `Ctrl+C` y decida cuándo y cómo cerrarse en su lugar.
