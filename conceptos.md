...
## Jerarquía de memoria y DMA

| Nivel | Resumen | Qué se ve |
|---|---|---|
| 1 | No toda la memoria es igual, hay niveles. | Analogía mesa-cajón-armario-almacén, velocidad vs capacidad y coste. |
| 2 | Registros, caché, RAM y disco; los dispositivos pueden escribir en RAM sin pasar por la CPU. | Localidad temporal/espacial, ejemplo matriz por filas vs columnas. DMA vs que la CPU copie dato a dato (*polling*). |
| 3 | Aciertos/fallos de caché, TLB, *page cache*. | `perf stat` (cache-misses, page-faults), impacto medible en tiempo de ejecución, *page cache* del sistema de ficheros. DMA en discos/red (p.ej. `dmesg`, driver que programa el DMA y atiende la IRQ de fin de transferencia). |
